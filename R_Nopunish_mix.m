...
%   
%
function Q = R_Nopunish_mix(MueLocation,fbsCount, NumRealization, QTable)
...
%% Generate the UEs
selectedMUE(1) = UE(204, 207);
selectedMUE(2) = UE(150, 150);
selectedMUE(3) = UE(-200, 0);
mue = selectedMUE(MueLocation);
...
%         R = K - dum2*dum1;
        for j=1:size(FBS,2)
            ...
            beta = fbs.dMUE/dth;
            % check if any UE has capacity lower than threshold
            if (check_fue==0 && check_mue==0)
...
%         end
    ...
    QFinal = answer;
    save(sprintf('nopunish/R_nopunish_mix_L:%d,%d,Real:%d.mat',MueLocation,fbsCount, NumRealization),'QFinal');
