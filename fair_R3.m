...
%
function Q = fair_R3(mueNumber,fbsCount,NumRealization, QTable)
...
for i=1:3
%     if i<= fbsCount
        FBS_Max{i} = FemtoStation_3S(180+(i-1)*35,150, BS, mue, 10);
%     end
end

for i=1:3
%     if i+3<= fbsCount
        FBS_Max{i+3} = FemtoStation_3S(165+(i-1)*30,180, BS, mue, 10);
%     end
end

for i=1:4
%     if i+6<= fbsCount
        FBS_Max{i+6} = FemtoStation_3S(150+(i-1)*35,200, BS, mue, 10);
%     end
end

for i=1:3
%     if i+10<= fbsCount
        FBS_Max{i+10} = FemtoStation_3S(160+(i-1)*35,240, BS, mue, 10);
%     end
end

for i=1:3
%     if i+13<= fbsCount
        FBS_Max{i+13} = FemtoStation_3S(150+(i-1)*35,280, BS, mue, 10);
%     end
end
%%
% 
...
if fbsCount>=16, FBS{16} = FBS_Max{13}; end
    ...
    selectedMUE.SINR = SINR_MUE_3(FBS, BS, selectedMUE, -120, 1000);
    ...
    for episode = 1:Iterations
        ...
        selectedMUE = selectedMUE.setCapacity(log2(1+SINR_MUE_3(FBS, BS, selectedMUE, -120, NumRealization)));
...
%         R = K - (selectedMUE.SINR - sinr_th)^2;
        ...
        if sum(sum(abs(Q1-QTable)))<10 && sum(sum(QTable >0))
            if count>1000
    ...
    QFinal = answer;
% end
save(sprintf('R3/R3-shared:%d,%d,%d.mat',mueNumber,fbsCount, NumRealization),'QFinal');
